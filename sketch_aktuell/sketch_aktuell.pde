...
void loadData() {
  ...
  for (int k = 0; k<table.getRowCount(); k++) {
    ...
    TableRow firstRow = table.getRow(1);
    ...
    // println(mitarbeiterProfil+" "+mitarbeiterProfilBefore);
     println("firstday: "+firstDay+" datum: "+datum+" clicker: "+clicker);
    datum = row.getInt("Datum (Tagnummer)"); 
    
    firstDay = firstRow.getInt("Datum (Tagnummer)"); 
    ...
    // Abfrage nach wunscherfüllung, wenn nicht gegeben wird mitarbeiter rot markiert
